...
export default function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    ...
    // Initialize a unique user identifier for Supabase progress tracking.
    let uid = null;
    if (typeof window !== 'undefined') {
      ...
      if (!uid) {
        try {
          ...
          // Fallback to a timestamp-based ID if crypto API is unavailable.
          uid = 'user-' + Date.now();
        }
  ...
  return <Component {...pageProps} theme={theme} toggleTheme={toggleTheme} userId={userId} />;
}